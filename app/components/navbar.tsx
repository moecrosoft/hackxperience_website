...
export default function Navbar() {
  ...
  const handleNavClick = (item: NavItem) => {
    if ("action" in item && item.action === "game") {
      setIsGameOpen(true);
      return;
    }
    if ("href" in item) {
  ...
  return (
    <>
      <nav
        className={`${ibmPlexMono.className} fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-3 sm:px-6 md:px-10 h-11 transition-shadow duration-300 ${
      ...
      >
        ...
        <motion.button
          ...
          className="flex items-center gap-0 text-white text-[12px] xs:text-[13px] sm:text-[14px] font-bold tracking-wide cursor-pointer whitespace-nowrap"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <motion.button
            ...
            className="login_secure bg-[#c00000] text-white text-[10px] xs:text-[11px] sm:text-[12px] font-bold tracking-[0.10em] px-3 sm:px-4 py-1.5 cursor-pointer whitespace-nowrap"
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="md:hidden absolute top-11 left-0 right-0 flex flex-col border-t border-white/10 overflow-hidden shadow-2xl"
              style={{ backgroundColor: "#1d1c17" }}
            >
              {navItems.map((item, i) => (
                <motion.button
